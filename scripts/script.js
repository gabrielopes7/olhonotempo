...
if (!localStorage.getItem("dia1")) {
  ...
  fetch(url)
    ...
    .then((dados) => {
      ...
      dadosClima[5].innerText = `${amanha.dia_semana} - ${dataAmanha}`;
      ...
      dadosProximos[0].innerText = `${proximoDiaUm.dia_semana} - ${dataProximoDiaUm}`;
      ...
      dadosProximos[5].innerText = `${proximoDiaDois.dia_semana} - ${dataProximoDiaDois}`;
      ...
      dadosProximos[10].innerText = `${proximoDiaTres.dia_semana} - ${dataProximoDiaTres}`;
...
function cidadePrevisao(cidade) {
  let url2 = "https://apiprevmet3.inmet.gov.br/previsao/" + cidade.toString(); // Recebe o parametro da função acima e faz a junção com a API do Inmet, fazendo a solicitação da cidade desejada.
  fetch(url2)
    .then((res) => {
      return res.json();
    })
    .then((dados) => {
      ...
      let proximoDiaDois = dados[cidade][`${dataProximoDiaDois}`]; // Pega a previsões das respectivas cidades.
...
function divHoje(hoje) {
  ...
  dadosClima[4].innerText = `Temperatura mínima: ${temp_min}`; // Faz a desconstrução do objeto para requisitar cada propriedade dele, mostrar na dela e salvar no localStorage.
...
function divAmanha(amanha) {
  ...
  dadosClima[9].innerText = `Temperatura mínima: ${temp_min}`; // Faz a desconstrução do objeto para requisitar cada propriedade dele, mostrar na dela e salvar no localStorage.
...
function divProximoUm(diaUm) {
  ...
  dadosProximos[4].innerText = `Temperatura mínima: ${temp_min}`; // Faz a desconstrução do objeto para requisitar cada propriedade dele, mostrar na dela e salvar no localStorage.
...
function divProximoDois(diaDois) {
  ...
  dadosProximos[9].innerText = `Temperatura mínima: ${temp_min}`; // Faz a desconstrução do objeto para requisitar cada propriedade dele, mostrar na dela e salvar no localStorage.
...
function divProximoTres(diaTres) {
  ...
  dadosProximos[14].innerText = `Temperatura mínima: ${temp_min}`; // Faz a desconstrução do objeto para requisitar cada propriedade dele, mostrar na dela e salvar no localStorage.
